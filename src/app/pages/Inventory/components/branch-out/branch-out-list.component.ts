...
@Component({
  selector: 'branch-out-list',
  templateUrl: './branch-out-list.template.html',
  providers: [AuthService],
  styleUrls: ["../../../Style.css", "../../../modal-style.css"],
})
export class BranchOutListComponent {
  ...
  constructor(private masterService: MasterRepo, private _authService: AuthService, private _router: Router, public dialog: MdDialog) {
    try {
      ...
      this.masterService.getTrnMainList('TO')
        .subscribe(res => {
  ...
  onAddClick() {
    try {
      this._router.navigate(["/pages/inventory/branch-out/add-branch-out", { vt: 5, mode: "add", returnUrl: this._router.url }]);
    } catch (ex) {
      //console.log(ex);
      alert(ex);
    }
  ...
  onEditClick(event): void {
    try {
      this._router.navigate(["/pages/inventory/branch-out/add-branch-out", { vt: 5, vchr: event.data.VCHRNO, returnUrl: this._router.url, div: event.data.DIVISION, phiscal: event.data.PhiscalID, mode: "edit" }]);
    } catch (ex) {
      //console.log(ex);
      alert(ex);
    }
  ...
  onViewClick(event): void {
    try {
      this._router.navigate(["/pages/inventory/branch-out/add-branch-out", { vt: 5, vchr: event.data.VCHRNO, returnUrl: this._router.url, div: event.data.DIVISION, phiscal: event.data.PhiscalID, mode: "view" }]);
    } catch (ex) {
      //console.log(ex);
      alert(ex);
    }
