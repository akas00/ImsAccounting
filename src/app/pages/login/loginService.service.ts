...
export class LoginService implements OnInit, OnDestroy {
    ...
    oldPassword: string;
    username: string
    constructor(private http: Http, private gblstate: GlobalState, private authSerice: AuthService, private router: Router, private dbWrapper: IndexedDbWrapper,private alertService:AlertService) {
        ...
        return this.http.post(this.apiUrl + '/jwt', { username: username, password: password }, option)
            .map((response: Response) => {
                ...
                if (user && user.token) {
                    ...
                    if(user.license.message && user.license.message!=""){
                        setTimeout(()=>{
                            this.alertService.license(user.license.message, user.license.link);
                        },100)
    ...
    updatePassword(password: string, username: string) {
      ...
      public updatePhiscalYear(NewPhiscalYear:string){
        ...
        this.http.get(AppConfiguration.apiUrl+ '/updatePhiscalYearForAnonymous/'+NewPhiscalYear,this.authSerice.getRequestOption())
        .map(res=> res.json()).subscribe(data=>{
          if(data.status =='ok'){
            returnSubject.next(data);
            returnSubject.unsubscribe;
