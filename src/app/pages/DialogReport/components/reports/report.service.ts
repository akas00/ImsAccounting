...
export interface IReportParam {
    valueFrom: string;//for reportdrill keyword 'dialog' for value from dialog box and keyword 'table' for getting value from load table 
    param: string;
    value: any;
}
...
export class ReportService implements OnInit {
    ...
    public costcenterList: Array<CostCenter> = [];
    ...
    //     reportQuery:undefined
    // }]
    constructor(private http: Http, private authService: AuthService, private state: GlobalState, private MasterService: MasterRepo, private setting: SettingService, private adduserService: AddUserService) {
...
if (this.divisionList.length == 0) {
    ...
    public getProductList() {
        if (this.productList.length == 0) {
            let dlist = [];
            this.MasterService.getProductList().subscribe(res => {
                dlist.push(<any>res);
                this.divisionList = dlist;
            });
        }
    ...
    public loadReportData(RData: any) {
        ...
        this.http.post(this.apiUrl + '/loadreportdata', RData, this.getRequestOption())
            .subscribe(response => {
                ...
                //let err=error.json()
                res.status = 'error'; res.result = error.json();
                returnSubject.next(res);
                returnSubject.unsubscribe();
            }
            );
